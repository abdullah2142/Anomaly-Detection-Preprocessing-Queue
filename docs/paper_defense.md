...
We have since measured both missing cases (§5.7). Preprocessing an undegraded image costs −14.66 pp (PaDiM) and −15.12 pp (PatchCore) pooled across the six methods, over four times the −3.35 pp penalty of matched restoration on genuinely degraded input — so unnecessary preprocessing, not mistaken preprocessing, is the dominant risk. Misidentifying the degradation costs −5.17 to −7.57 pp against doing nothing. Our one positive recommendation survives that test where it was made: CLAHE on undegraded images costs PatchCore −0.95 pp ($p = 0.2031$), indistinguishable from zero, so it does not need a gating detector. It costs PaDiM −4.92 pp ($p = 0.0156$), so we confine the recommendation to PatchCore.

...
2. **Test-time rescue preprocessing is net-harmful** in all 4 conditions (−4.6 to −14.9 pp, category-clustered $p \le 2.4\times10^{-4}$), *even when the corruption type, severity, and restoration parameters are known exactly* — the *preprocessing fallacy* — because restoration algorithms produce a third distribution containing novel artifacts that anomaly detectors score as defects.

3. **Rescue harm is method-specific and severe**: Wiener deconvolution (up to −37 pp via mismatched kernel ringing, and ~−12.5 pp even with an exact oracle kernel) and Retinex (−3 to −11 pp via colour distortion) are consistently harmful; CLAHE is the only conditionally safe method for PatchCore under low-light due to its spatial frequency-preserving contrast adjustment.

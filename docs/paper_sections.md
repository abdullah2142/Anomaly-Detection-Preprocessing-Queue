...
Table Z reports rescue success rates and mean AUROC deltas across the 810 rescue instances per condition (effective N after dropping zero differences: 645–806, see §Limitations). In all four conditions, mean rescue delta is significantly negative (category-clustered exact permutation test, all $p \le 2.4\times10^{-4}$):
...
2. **Test-time rescue preprocessing is net-harmful** in all 4 conditions (−4.6 to −14.9 pp, category-clustered $p \le 2.4\times10^{-4}$) — the *preprocessing fallacy* — because restoration algorithms produce a third distribution containing novel artifacts that anomaly detectors score as defects.

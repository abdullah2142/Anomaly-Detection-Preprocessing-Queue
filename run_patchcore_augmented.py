...
OUTPUT_FILE = "results/patchcore_augmented.csv"
PARTIAL_FILE = "results/patchcore_augmented_partial.csv"

print(f"Script started at {time.ctime(START_TIME)}")
print(f"Graceful timeout set to {TIMEOUT_SECONDS / 3600:.1f} hours.")
print("MODE: Augmented training (50% random corruption) + full degradation + rescue at all severities")
...
# ---------------------------------------------------------------------------
# 4b. Augmented Training Dataset Wrapper
# ---------------------------------------------------------------------------
class AugmentedTrainDataset(Dataset):
    """Wraps a clean training dataset, randomly applying a corruption to 50% of
    images so PatchCore's memory bank covers both clean and degraded patch features
    — simulating unknown real-world degradation."""
    CTYPES = ["low_light", "gaussian_blur", "motion_blur", "sensor_noise", "fog_haze"]
    SEVS   = ["mild", "moderate", "severe"]

    def __init__(self, base_dataset, config, aug_prob=0.5):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() >= self.aug_prob:
            return item  # 50% chance: return clean
        ctype = random.choice(self.CTYPES)
        sev   = random.choice(self.SEVS)
        if dataclasses.is_dataclass(item):
            image = item.image
        else:
            image = item["image"]
        if isinstance(image, torch.Tensor):
            ...
            img_np = np.array(image).astype(np.uint8)
        corrupted = apply_corruption(img_np, ctype, sev, self.config, seed=idx)
        final_tensor = torch.from_numpy(corrupted).permute(2, 0, 1).float() / 255.0
        if dataclasses.is_dataclass(item):
            return dataclasses.replace(item, image=final_tensor)
        else:
            item["image"] = final_tensor
...
possible_resume_paths = [OUTPUT_FILE, PARTIAL_FILE, "/kaggle/input/anomaly-detection-results/patchcore_final.csv"]
...
for category in CATEGORIES:
    for seed in SEEDS:
        ...
        try:
            print(f"[PHASE 1] Augmented Training (aug_prob=0.5, random ctype+severity)...")
            datamodule.setup(stage="fit")
            aug_train  = AugmentedTrainDataset(datamodule.train_data, config, aug_prob=0.5)
            aug_loader = DataLoader(aug_train, batch_size=32, num_workers=2,
                                    collate_fn=datamodule.train_data.collate_fn, shuffle=True)
            engine.fit(model=model, train_dataloaders=aug_loader,
            ...
            for ctype, severities in config["corruptions"].items():
                for sev in severities.keys():
                    ...
                    all_results.append({"model": "PatchCore", "category": category, "seed": seed, "phase": "degradation", "ctype": ctype, "severity": sev, "rescue": "none", "image_AUROC": deg_auroc})
                    save_results()

                    if ctype in RESCUE_MAP:  # rescue at ALL severities for 4-way comparison
                        for r_name, r_func in RESCUE_MAP[ctype]:
                            print(f"[PHASE 3] Rescue: {ctype} ({sev}) + {r_name}")
                            # Fresh base + rescue func applied on top of corruption
                            ...
                            all_results.append({"model": "PatchCore", "category": category, "seed": seed, "phase": "rescue", "ctype": ctype, "severity": sev, "rescue": r_name, "image_AUROC": res_auroc})
                            save_results()
        except Exception as e:
            print(f"❌ Error in {category}/{seed}: {e}")
        
        # Rule 2: Explicit cleanup after every category/seed
        del model
        del engine
        cleanup_memory()

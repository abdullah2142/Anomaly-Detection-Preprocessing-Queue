# %% [markdown]
# # NB-7a: Preprocessing Rescue — PaDiM (Part 1: Low-light, Blur)
# **GPU**: ON | **Internet**: On | **Time**: ~7-9 hrs
#
# **Input Data Required**:
# - MVTec-AD dataset
# - NB-0 output (`experiment_config.json`)
#
# Same structure as NB-6a but uses PaDiM model.
...
for category in CATEGORIES:
    for seed in SEEDS:
        ...
        pd.DataFrame(results).to_csv(
            "/kaggle/working/preprocessing/padim_preprocess_blur_light_WIP.csv", index=False)
...
df.to_csv("/kaggle/working/preprocessing/padim_preprocess_blur_light.csv", index=False)
pivot = df.groupby(["corruption_type", "severity", "preprocessing"])["image_AUROC"].agg(["mean", "std"]).round(4)
print("\nPaDiM Preprocessing Results (blur/light)")
print("="*70)
print(pivot.to_string())
print(f"\nWall time: {(time.time()-start_time)/3600:.2f}h")
print("📋 NEXT: Save & Run All → NB-7b")

# %% [markdown]
# # NB-6b: Preprocessing Rescue — PatchCore (Part 2: Noise, Fog, Combined)
# **GPU**: ON | **Internet**: On | **Time**: ~9-12 hrs
#
# **Input Data Required**:
# - MVTec-AD dataset
...
# - sensor_noise (3 severities × 2 methods: NLM, HistMatch) — skip BM3D for time
# - fog_haze (3 severities × 2 methods: DarkChannelPrior, HistMatch)
# - combined (1 severity × 3 methods: Denoise→CLAHE, CLAHE→Denoise, HistMatch)
# Total: 15 variants across 15 categories × 3 seeds
#
# ⚠️ BM3D is skipped by default (too slow: ~2s/image). Set RUN_BM3D=True below to include it.
...
RUN_BM3D = False  # Set True to include BM3D (adds ~5hrs)
...
# %% [markdown]
# ## Cell 4: Define Experiment Variants
...
for sev in ["mild", "moderate", "severe"]:
    ...
    if RUN_BM3D:
        EXPERIMENT_VARIANTS.append(("sensor_noise", sev, "BM3D"))
...
for category in CATEGORIES:
    for seed in SEEDS:
        ...
        for ctype, sev, preprocess_name in EXPERIMENT_VARIANTS:
            ...
            elif preprocess_name == "BM3D":
                import bm3d as bm3d_lib
                processed = []
                for img in corrupted:
                    img_f = img.astype(np.float64) / 255.0
                    sigma = np.mean(estimate_sigma(img_f, channel_axis=-1))
                    d = bm3d_lib.bm3d(img_f, sigma_psd=sigma)
                    processed.append(np.clip(d * 255, 0, 255).astype(np.uint8))
        ...
        pd.DataFrame(results).to_csv(
            "/kaggle/working/preprocessing/patchcore_preprocess_noise_fog_WIP.csv", index=False)
...
df.to_csv("/kaggle/working/preprocessing/patchcore_preprocess_noise_fog.csv", index=False)

pivot = df.groupby(["corruption_type", "severity", "preprocessing"])["image_AUROC"].agg(["mean", "std"]).round(4)
print("\nPreprocessing Results (PatchCore — noise/fog/combined)")
print("="*70)
print(pivot.to_string())
print(f"\nWall time: {(time.time()-start_time)/3600:.2f}h")
print("📋 NEXT: Save & Run All → NB-8 (Training corruption) or NB-10 (Analysis)")

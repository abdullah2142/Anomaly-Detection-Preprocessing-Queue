...
OUTPUT_FILE = "results/patchcore_rescue_mod_sev.csv"   # CHANGED
PARTIAL_FILE = "results/patchcore_rescue_partial.csv"  # CHANGED

print(f"Script started at {time.ctime(START_TIME)}")
print(f"Graceful timeout set to {TIMEOUT_SECONDS / 3600:.1f} hours.")
print("MODE: Degradation + Rescue at MODERATE and SEVERE only (mild skipped)")
...
possible_resume_paths = [OUTPUT_FILE, PARTIAL_FILE]
...
TARGET_SEVERITIES = {"moderate", "severe"}  # CHANGED: skip mild
...
for category in CATEGORIES:
    for seed in SEEDS:
        ...
        try:
            print(f"[PHASE 1] Training...")  # CHANGED: no baseline test call
            engine.fit(model=model, datamodule=datamodule)
            
            for ctype, severities in config["corruptions"].items():
                for sev in severities.keys():
                    if sev not in TARGET_SEVERITIES:  # CHANGED: skip mild
                        continue
                    ...
                    if ctype in RESCUE_MAP and sev in TARGET_SEVERITIES:  # CHANGED: moderate+severe

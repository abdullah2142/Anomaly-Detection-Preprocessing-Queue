...
- **Statistical validation**: ✅ Wilcoxon signed-rank tests with Benjamini-Hochberg FDR correction
...
**Core findings:**
1. **Augmented training significantly improves robustness** (+12.3 pp PatchCore, +10.1 pp PaDiM, p < 0.001).
...
Paired Wilcoxon signed-rank tests were performed on the unified dataset with BH-FDR correction. Full methodology and results in `notebooks/10_wilcoxon_testing.ipynb`.

### Augmentation Gains (H₁: Clean < Augmented)
* **PatchCore** (MVTec-AD): Mean Gain = **+12.3 pp** (p < 0.001)
* **PaDiM** (MVTec-AD): Mean Gain = **+10.1 pp** (p < 0.001)
* **PatchCore** (VisA): Mean Gain = **+17.8 pp** (p < 0.001)
* **PaDiM** (VisA): Mean Gain = **+13.9 pp** (p < 0.001)

### Rescue Deltas (Two-sided Wilcoxon)
All four conditions show significantly negative rescue deltas (p < 0.001).

### Per-Method Breakdown (FDR-corrected)
* **Wiener Deconvolution**: Catastrophically harmful (q < 0.001). Oracle-PSF severe: ~−12.5 pp.
* **Retinex**: Consistently harmful (q < 0.001, −3 to −11 pp).
* **CLAHE**: Only conditionally safe method — near-neutral for PatchCore under low-light.
* **NLM Denoise** and **DCP Dehaze**: Mild but significant harm (−1 to −5 pp).
...
| `09_wilcoxon_per_method_heatmap.png` | Per-method Wilcoxon FDR-corrected results |
...
- **VisA**: 12 categories (Kaggle: `marquis03/visa-dataset`) — Clean training: 12 categories; Augmented training: 4 categories (candle, cashew, pcb1, pipe_fryum)
